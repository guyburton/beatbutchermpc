...
class MultiBorderLayout extends BorderLayout {
	...
	Vector<Component> north = new Vector<Component>();

	Vector<Component> south = new Vector<Component>();

	Vector<Component> west = new Vector<Component>();

	Vector<Component> east = new Vector<Component>();

	Vector<Component> center = new Vector<Component>();
		...
		synchronized (target.getTreeLock()) {
			...
			if (south.size() > 0) {
				for (int i = 0; i < south.size(); i++) {
					...
					Dimension d = c.getPreferredSize();
					c.setSize(right - left, d.height);
					c.setBounds(left, bottom - d.height, right - left, c
							.getHeight());
					bottom -= d.height;
				}
			}
			if (east.size() > 0) {
				for (int i = 0; i < east.size(); i++) {
					...
					c.setBounds(right - d.width, top, c.getWidth(), bottom
							- top);
					right -= d.width;
				}
			}
